...
def init_visualizations(hps, model, logdir):
    ...
    def draw_samples(prefix):
        ...
        for t in temperatures:
            x_samples.append(sample_batch(y, [t] * n_batch))
        # x_samples.append(sample_batch(y, [.0]*n_batch))
        # x_samples.append(sample_batch(y, [.25]*n_batch))
        # x_samples.append(sample_batch(y, [.5]*n_batch))
        # x_samples.append(sample_batch(y, [.6]*n_batch))
        ...
        for i in range(len(x_samples)):
            x_sample = np.reshape(
                x_samples[i], (n_batch, hps.image_size, hps.image_size, 3))
            graphics.save_raster(x_sample, os.path.join(logdir, '{}sample_{}.png'.format(prefix, i)))
...
def main(hps):
    ...
    visualise = init_visualizations(hps, model, os.path.dirname(hps.restore_path))
    visualise(prefix='')
